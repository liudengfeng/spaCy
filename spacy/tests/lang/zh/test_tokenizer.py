...
JIEBA_TOKENIZER_TESTS = [
    (TEXTS[0], [
        '作为', '语言', '而言', '，', '为', '世界', '使用', '人数', '最多', '的', '语言', '，',
        '目前', '世界', '有', '五', '分', '之', '一', '人口', '做为', '母语', '。'
    ]),
]
PKUSEG_TOKENIZER_TESTS = [
    (TEXTS[0], [
        '作为', '语言', '而言', '，', '为', '世界', '使用', '人数', '最多', '的', '语言', '，',
        '目前', '世界', '有', '五分之一', '人口', '做为', '母语', '。'
    ]),
]
# fmt: on
...
def test_zh_tokenizer_tct_offline(zh_tokenizer_tct_offline, text,
                                  expected_tokens):
    """测试离线分词"""
    tokens = [token.text for token in zh_tokenizer_tct_offline(text)]
    assert tokens == expected_tokens


@pytest.mark.parametrize("text,expected_tokens", JIEBA_TOKENIZER_TESTS)
def test_zh_tokenizer_tct_online(zh_tokenizer_tct_online, text,
                                 expected_tokens):
    """测试在线分词"""
    tokens = [token.text for token in zh_tokenizer_tct_online(text)]
    assert tokens == expected_tokens


@pytest.mark.skip(reason="使用腾讯分词")
...
def test_zh_tokenizer_pkuseg_user_dict(zh_tokenizer_pkuseg):
    user_dict = _get_pkuseg_trie_data(
